...
def wf_ccl(
    ...
    assert dndz_tuple[0].shape[0] == dndz_tuple[1].shape[0], (
        'dndz must be a tuple of two arrays of shape len(z_grid) '
    ...
    if probe == 'lensing':
        ...
        if ia_bias_tuple is None:
            ia_bias_1d = build_ia_bias_1d_arr(
                z_grid_out=z_grid,
                cosmo_ccl=cosmo_ccl,
        ...
        assert ia_bias_tuple[0].shape == ia_bias_tuple[1].shape, (
            'ia_bias must be a tuple of two arrays of len(z_grid)'
        )
        ...
        wf_lensing_arr = np.asarray(
            [
                wf_lensing_obj[zbin_idx].get_kernel(comoving_distance)
                for zbin_idx in range(zbins)
            ]
        )
        ...
        assert gal_bias_tuple[0].shape[0] == len(z_grid), (
            'gal_bias_tuple[0] must have shape len(z_grid)'
        )
        assert gal_bias_tuple[1].shape == (len(z_grid), zbins), (
            'gal_bias_tuple[1] must have shape (len(z_grid), zbins)'
        )

        if mag_bias_tuple is not None:
            assert len(mag_bias_tuple) == 2, (
                'mag_bias_tuple must be a tuple of length 2'
            )
            assert mag_bias_tuple[0].shape[0] == len(z_grid), (
                'mag_bias_tuple[0] must have shape len(z_grid)'
            )
            assert mag_bias_tuple[1].shape == (len(z_grid), zbins), (
                'mag_bias_tuple[1] must have shape (len(z_grid), zbins)'
            )
        ...
        wf_galaxy_arr = np.asarray(
            [
                wf_galaxy_obj[zbin_idx].get_kernel(comoving_distance)
                for zbin_idx in range(zbins)
            ]
        )
        ...
        if which_wf == 'with_galaxy_bias':
            ...
            raise ValueError(
                'which_wf must be "with_galaxy_bias", "without_galaxy_bias" or "galaxy_bias_only"'
            )
...
def wf_galaxy_ccl(
    ...
    if gal_bias_2d_array is None:
        warnings.warn(
            "the bias implementation should be improved, it's not very clean. read everything"
            ' from the fiducals dict!!'
        )
        z_values = np.asarray(
            [fiducial_params[f'zmean{zi:02d}_photo'] for zi in range(1, zbins + 1)]
        )
        bias_values = np.asarray(
            [fiducial_params[f'b{zi:02d}_photo'] for zi in range(1, zbins + 1)]
        )
        gal_bias_2d_array = build_galaxy_bias_2d_arr(
            bias_values, z_values, zbins, z_grid, bias_model
        )
    elif np.all(gal_bias_2d_array == 1):  # i.e., no galaxy bias
        gal_bias_2d_array = np.ones((len(z_grid), zbins))

    assert gal_bias_2d_array.shape == (len(z_grid), zbins), (
        'gal_bias_2d_array must have shape (len(z_grid), zbins)'
    )

    # redshift distribution
    if dndz is None:
        niz_unnormalized_arr = np.asarray(
            [niz_unnormalized_analytical(z_grid, zbin_idx) for zbin_idx in range(zbins)]
        )
        niz_normalized_arr = normalize_niz_simps(
            niz_unnormalized_arr, z_grid
        ).T  # ! unnecessary to normalize
        dndz = (z_grid, niz_normalized_arr)
        assert niz_normalized_arr.shape == (len(z_grid), zbins), (
            'niz_normalized_arr must have shape (len(z_grid), zbins)'
        )

    assert len(dndz) == 2, 'dndz must be a tuple of length 2'
    assert dndz[0].shape[0] == dndz[1].shape[0], (
        'dndz must be a tuple of two arrays of shape len(z_grid) and '
    ...
    wig = [
        ccl.tracers.NumberCountsTracer(
            cosmo,
            has_rsd=False,
            dndz=(dndz[0], dndz[1][:, zbin_idx]),
            bias=(z_grid, gal_bias_2d_array[:, zbin_idx]),
            mag_bias=None,
            n_samples=n_samples,
        )
        for zbin_idx in range(zbins)
    ...
    wig_nobias_PyCCL_arr = np.asarray(
        [wig[zbin_idx].get_kernel(chi) for zbin_idx in range(zbins)]
    )
...
def wf_lensing_ccl(
    ...
    assert ia_bias[0].shape == ia_bias[1].shape, (
        'ia_bias must be a tuple of two arrays of the same shape'
    )
    ...
    a_arr = 1 / (1 + z_grid)
    chi = ccl.comoving_radial_distance(cosmo, a_arr)
    wf_lensing = np.asarray(
        [wf_lensing_obj[zbin_idx].get_kernel(chi) for zbin_idx in range(zbins)]
    )
...
# ! start new parallel
def cls_and_derivatives_parallel_old(
    ...
    for param_to_vary in list_params_to_vary:
        assert param_to_vary in fiducial_values_dict.keys(), (
            f'{param_to_vary} is not in the fiducial values dict'
        )
        ...
        varied_param_values = (
            fiducial_values_dict[param_to_vary]
            + fiducial_values_dict[param_to_vary] * percentages
        )
        if param_to_vary == 'wa':
        ...
        results = Parallel(n_jobs=-1, backend='threading')(
            delayed(cl_parallel_helper_old)(
                param_to_vary,
                variation_idx,
                varied_fiducials,
                fiducial_values_dict,
                ...
                bias_model,
                pk=pk,
                use_only_flat_models=use_only_flat_models,
            )
        ...
        print(
            f'param {param_to_vary} Cls computed in {(time.perf_counter() - t0):.2f} seconds'
        )

        dcl_LL[param_to_vary] = stem(
            cl_LL[param_to_vary], varied_param_values, zbins, nbl_WL
        )
        dcl_GL[param_to_vary] = stem(
            cl_GL[param_to_vary], varied_param_values, zbins, nbl_GC
        )
        dcl_GG[param_to_vary] = stem(
            cl_GG[param_to_vary], varied_param_values, zbins, nbl_GC
        )

    return cl_LL, cl_GL, cl_GG, dcl_LL, dcl_GL, dcl_GG


def cl_parallel_helper_old(
    param_to_vary,
    variation_idx,
    varied_fiducials,
    fiducial_values_dict,
    ...
    try:
        if use_only_flat_models:
            assert fiducial_values_dict['Om_k0'] == 0, (
                'if use_only_flat_models is True, Om_k0 must be 0'
            )
            assert varied_fiducials['Om_k0'] == 0, (
                'if use_only_flat_models is True, Om_k0 must be 0'
            )
            assert 'Om_k0' not in list_params_to_vary, (
                'if use_only_flat_models is True, Om_k0 must not be in list_params_to_vary'
            )

            if param_to_vary == 'Om_Lambda0':
                varied_fiducials['Om_m0'] = 1 - varied_fiducials['Om_Lambda0']
        else:
            varied_fiducials['Om_k0'] = (
                1 - varied_fiducials['Om_m0'] - varied_fiducials['Om_Lambda0']
            )
            if np.abs(varied_fiducials['Om_k0']) < 1e-8:
                varied_fiducials['Om_k0'] = 0

        varied_fiducials['Om_nu0'] = cosmo_lib.get_omega_nu0(
            varied_fiducials['m_nu'],
            varied_fiducials['h'],
            n_ur=None,
            n_eff=varied_fiducials['N_eff'],
            n_ncdm=None,
            neutrino_mass_fac=None,
            g_factor=None,
        )

        cosmo_ccl = cosmo_lib.instantiate_cosmo_ccl_obj(
            varied_fiducials, extra_parameters
        )
        assert (
            varied_fiducials['Om_m0'] / cosmo_ccl.cosmo.params.Omega_m - 1
        ) < 1e-7, 'Om_m0 is not the same as the one in the fiducial model'

        wl_kernel = wf_lensing_ccl(
            z_grid,
            'with_IA',
            cosmo=cosmo_ccl,
            dndz=dndz,
            ia_bias=None,
            A_IA=varied_fiducials['A_IA'],
        ...
        cl_LL_part = cl_PyCCL(
            wl_kernel, wl_kernel, ell_LL, zbins, p_of_k_a=pk, cosmo=cosmo_ccl
        )
        cl_GL_part = cl_PyCCL(
            gc_kernel, wl_kernel, ell_GL, zbins, p_of_k_a=pk, cosmo=cosmo_ccl
        )
        cl_GG_part = cl_PyCCL(
            gc_kernel, gc_kernel, ell_GG, zbins, p_of_k_a=pk, cosmo=cosmo_ccl
        )
...
def compute_cls_derivatives(
    ...
    if use_only_flat_models:
        assert cfg['cosmology']['other_params']['Om_k0'] == 0, (
            'if use_only_flat_models is True, Om_k0 must be 0'
        )
        assert 'Om_k0' not in list_params_to_vary, (
            'if use_only_flat_models is True, Om_k0 must not be in list_params_to_vary'
    ...
    for name_par_tovary in list_params_to_vary:
        assert name_par_tovary in free_fid_pars_dict.keys(), (
            f'{name_par_tovary} is not in the fiducial values dict'
        )
        ...
        # shift the parameter
        varied_param_values = (
            free_fid_pars_dict[name_par_tovary]
            + free_fid_pars_dict[name_par_tovary] * percentages
        )
        if (
            free_fid_pars_dict[name_par_tovary] == 0
        ):  # wa is 0! take directly the percentages
        ...
        # ! benchmark
        results = [
            cl_derivatives_helper(
                name_par_tovary=name_par_tovary,
            ...
            for varied_fid_pars_dict[name_par_tovary] in tqdm(varied_param_values)
        ]
        ...
        print(
            f'param {name_par_tovary} Cls computed in {(time.perf_counter() - t0):.2f} seconds'
        )

        try:
            dcl_LL[name_par_tovary] = stem(
                cl_LL[name_par_tovary], varied_param_values, zbins, nbl_WL, 1
            )
            dcl_GL[name_par_tovary] = stem(
                cl_GL[name_par_tovary], varied_param_values, zbins, nbl_GC, 1
            )
            dcl_GG[name_par_tovary] = stem(
                cl_GG[name_par_tovary], varied_param_values, zbins, nbl_GC, 1
            )
        except np.linalg.LinAlgError as e:
            print(e)
            print(
                'SteM derivative computation failed, increasing tolerance from 1% to 6%'
            )
            dcl_LL[name_par_tovary] = stem(
                cl_LL[name_par_tovary], varied_param_values, zbins, nbl_WL, 6
            )
            dcl_GL[name_par_tovary] = stem(
                cl_GL[name_par_tovary], varied_param_values, zbins, nbl_GC, 6
            )
            dcl_GG[name_par_tovary] = stem(
                cl_GG[name_par_tovary], varied_param_values, zbins, nbl_GC, 6
            )
...
def cl_derivatives_helper(
    name_par_tovary,
    varied_fid_pars_dict,
    cl_LL,
    cl_GL,
    cl_GG,
    cfg,
    ...
    for cosmo_par in varied_fid_pars_dict.keys():
        if cosmo_par != name_par_tovary:
            assert (
                fid_pars_dict['FM_ordered_params'][cosmo_par]
                == varied_fid_pars_dict[cosmo_par]
            ), f'{cosmo_par} is not the same as in the fiducial model'
    ...
    gal_bias_polyfit_values = [
        varied_fid_pars_dict[f'bG{zi:02d}'] for zi in range(1, 5)
    ]
    mag_bias_polyfit_values = [
        varied_fid_pars_dict[f'bM{zi:02d}'] for zi in range(1, 5)
    ]
    mult_shear_bias_values = [
        varied_fid_pars_dict[f'm{zi:02d}'] for zi in range(1, zbins + 1)
    ]
    ...
    other_params['camb_extra_parameters']['camb']['HMCode_logT_AGN'] = (
        varied_fid_pars_dict['logT']
    )
    ...
    if cfg['covariance_cfg']['PyCCL_cfg']['which_pk_for_pyccl'] == 'CLOE':
        ...
        if name_par_tovary in names_cosmo_pars:
            ...
            cloe_pk_filename = cfg['general_cfg']['CLOE_pk_filename'].format(
                CLOE_pk_folder=cloe_pk_folder, param_name='w0', param_value=-1
            )
    ...
    # quick check
    assert (
        varied_fid_pars_dict['Om'] / ccl_obj.cosmo_ccl.cosmo.params.Omega_m - 1
    ) < 1e-7, 'Om_m0 is not the same as the one in the fiducial model'
    ...
    if general_cfg['which_forecast'] == 'SPV3':
        ...
        ccl_obj.set_gal_bias_tuple_istf(
            z_grid=z_grid_nz, bias_function_str=bias_func_str, bias_model=bias_model
        )
    ...
    cl_LL = ccl_obj.compute_cls(
        ell_LL, pk, ccl_obj.wf_lensing_obj, ccl_obj.wf_lensing_obj, 'spline'
    )
    cl_GL = ccl_obj.compute_cls(
        ell_GL, pk, ccl_obj.wf_galaxy_obj, ccl_obj.wf_lensing_obj, 'spline'
    )
    cl_GG = ccl_obj.compute_cls(
        ell_GG, pk, ccl_obj.wf_galaxy_obj, ccl_obj.wf_galaxy_obj, 'spline'
    )

    for ell_idx, _ in enumerate(ell_LL):
        for zi in range(zbins):
            for zj in range(zbins):
                cl_LL[ell_idx, zi, zj] *= (1 + mult_shear_bias_values[zi]) * (
                    1 + mult_shear_bias_values[zj]
                )
    ...
    if (
        varied_fid_pars_dict[name_par_tovary]
        == fid_pars_dict['FM_ordered_params'][name_par_tovary]
    ):

...
def get_julia_file_path():
    """Get the absolute path to the Julia file in the package."""
    try:
        # For Python 3.9+
        with resources.as_file(
            resources.files('spaceborne') / 'julia_integrator.jl'
        ) as julia_file:
            return str(julia_file)
    except AttributeError:
        # Fallback for older Python versions
        with resources.path('spaceborne', 'julia_integrator.jl') as julia_file:
            return str(julia_file)
...
def run(cfg):
    ...
    if use_h_units:
        ...
        _pk_txt_label = 'Mpcoverh3'
    else:
        k_txt_label = '1overMpc'
        _pk_txt_label = 'Mpc3'

    if not cfg['ell_cuts']['apply_ell_cuts']:
        _kmax_hmpc = cfg['ell_cuts']['kmax_h_over_Mpc_ref']
    ...
    pvt_cfg = {
        ...
        'z_grid': z_grid,
        'jl_integrator_path': get_julia_file_path(),
    }
    ...
    if cfg['BNT']['cl_BNT_transform'] or cfg['BNT']['cov_BNT_transform']:
        ...
        _z_means_ll = wf_cl_lib.get_z_means(z_grid, wf_gamma_ccl_bnt)
    else:
        bnt_matrix = None
    ...
    _wf_gamma = ccl_obj.wf_gamma_arr
    _wf_ia = ccl_obj.wf_ia_arr
    ...
    if cfg['BNT']['cl_BNT_transform']:
        ...
        _cl_ll_3d = cl_utils.cl_BNT_transform(ccl_obj.cl_ll_3d, bnt_matrix, 'L', 'L')
        _cl_3x2pt_5d = cl_utils.cl_BNT_transform_3x2pt(ccl_obj.cl_3x2pt_5d, bnt_matrix)
        warnings.warn(
    ...
    if compute_sb_ssc:
        ...
        if cfg['covariance']['which_pk_responses'] == 'halo_model':
            ...
            _r_mm = resp_obj.compute_r1_mm()
            resp_obj.set_su_resp(
                b2g_from_halomodel=True, include_b2g=cfg['covariance']['include_b2g']
            )
            _r_gm = resp_obj.r1_gm
            _r_gg = resp_obj.r1_gg
            _b1g_hm = resp_obj.b1g_hm
            _b2g_hm = resp_obj.b2g_hm

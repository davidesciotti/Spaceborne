...
# Loop through each weight function and plot
for wf_idx in range(len(wf_ccl_list)):
    ...
    plot_filename = os.path.join(output_dir, f"wf_plot_{wf_idx}.png")
...
fig, ax = plt.subplots(1, 3, figsize=(15, 5))  # Adjust figure size if needed
plt.tight_layout()

# Loop through the bins and plot the data
for zi in range(zbins):
    zj = zi  # Adjust this if needed
...
plot_filename = os.path.join(output_dir, "cls_plot.png")
plt.savefig(plot_filename, dpi=300)
...
fm_cfg = {
    ...
    'derivatives_folder': f"./input/DataVecDers/{flat_or_nonflat:s}/{which_pk:s}/{EP_or_ED:s}{zbins:02d}",
...
if not os.path.exists(fm_folder):
    os.makedirs(fm_folder)
    print(f"Folder '{fm_folder}' did not exist and has been created.")
else:
    print(f"Folder '{fm_folder}' already exists.")
...
for probe in probes:
    ...
    title = "%s_ell_max_%i_zbins_%s%i_zsteps_%s_%s_uncertainties" % (probe, lmax, ep_or_ed, zbins, len(z_grid), which_uncertainty)
...
plot_lib.bar_plot(np.array(list(perc_diff_probe.values())), title + "_percent_diff_G_SSC_cNG_over_G", (list(perc_diff_probe.keys())), nparams=nparams_toplot,

...
class SpaceborneCovariance():
    ...
    def __init__(self, cfg, zbins, ell_dict, bnt_matrix):
        ...
        if self.cov_ordering_2d == 'probe_ell_zpair':
            ...
            self.cov_4D_to_2D_3x2pt_func = mm.cov_4D_to_2DCLOE_3x2pt
            self.cov_4D_to_2D_3x2pt_func_kw = {'block_index': self.block_index, 'zbins': self.zbins}
            ...
            self.block_index = 'ell'
            self.cov_4D_to_2D_3x2pt_func = mm.cov_4D_to_2D
            self.cov_4D_to_2D_3x2pt_func_kw = {'block_index': self.block_index, 'optimize': True}
    ...
    def consistency_checks(self):
        ...
        if self.GL_OR_LG == 'LG':
            ...
            self.ind[self.zpairs_auto:(self.zpairs_auto + self.zpairs_cross), [2, 3]] = \
                self.ind[self.zpairs_auto:(self.zpairs_auto + self.zpairs_cross), [3, 2]]
    ...
    def reshape_cov(self, cov_in, ndim_in, ndim_out, nbl, zpairs=None, ind_probe=None, is_3x2pt=False):
        ...
        if ndim_out == 2:
            ...
            if is_3x2pt:
                ...
                cov_out = self.cov_4D_to_2D_3x2pt_func(cov_out, **self.cov_4D_to_2D_3x2pt_func_kw)
            else:
    ...
    def set_gauss_cov(self, ccl_obj, split_gaussian_cov):
        ...
        if split_gaussian_cov:
            ...
            self.cov_GC_g_6D = mm.covariance_einsum(
    ...
    def build_covs(self, ccl_obj, oc_obj):
        ...
        ----------
        ccl_obj : object
            PyCCL interface interface object containing PyCCL covariance terms, as well as cls 
        ...
        if self.g_code == 'OneCovariance':
            warnings.warn(
                "Implementation of OneCovariance G term is not complete: "
                "split terms and probe-specific ell binning missing"
            )
        ...
        if self.include_ssc:
            ...
            if self.ssc_code == 'Spaceborne':
                self.cov_3x2pt_ssc_10D = self._cov_8d_dict_to_10d_arr(self.cov_ssc_sb_3x2pt_dict_8D)
            elif self.ssc_code == 'PyCCL':
                self.cov_3x2pt_ssc_10D = self._cov_8d_dict_to_10d_arr(ccl_obj.cov_ssc_ccl_3x2pt_dict_8D)
        ...
        # # ! Select appropriate non-Gaussian covariance terms to include and from which code
        # if self.ssc_code == 'Spaceborne':
        ...
        #         cov_WL_g_6D = deepcopy(cov_3x2pt_g_10D[0, 0, 0, 0, :self.nbl_WL, :self.nbl_WL, :, :, :, :])
        #         cov_GC_g_6D = deepcopy(cov_3x2pt_g_10D[1, 1, 1, 1, :self.nbl_GC, :self.nbl_GC, :, :, :, :])
        #         cov_3x2pt_g_10D = deepcopy(cov_3x2pt_g_10D[:, :, :, :, :self.nbl_3x2pt, :self.nbl_3x2pt, :, :, :, :])
        ...
        #         cov_3x2pt_ng_10D = self._cov_8d_dict_to_10d_arr(ccl_obj.cov_ssc_ccl_3x2pt_dict_8D)

        #     elif self.covariance_cfg['PyCCL_cfg']['which_ng_cov'] == ['cNG', ]:
        #         cov_3x2pt_ng_10D = self._cov_8d_dict_to_10d_arr(ccl_obj.cov_cng_ccl_3x2pt_dict_8D)
        ...
        cov_WL_ssc_6D = deepcopy(self.cov_3x2pt_ssc_10D[0, 0, 0, 0, :self.nbl_WL, :self.nbl_WL, :, :, :, :])
        cov_WL_cng_6D = deepcopy(self.cov_3x2pt_cng_10D[0, 0, 0, 0, :self.nbl_WL, :self.nbl_WL, :, :, :, :])
        ...
        if self.cfg['BNT']['cov_BNT_transform']:
            ...
            cov_3x2pt_g_10D_dict = mm.cov_10D_array_to_dict(self.cov_3x2pt_g_10D, self.probe_ordering)
            cov_3x2pt_ssc_10D_dict = mm.cov_10D_array_to_dict(self.cov_3x2pt_ssc_10D, self.probe_ordering)
            cov_3x2pt_cng_10D_dict = mm.cov_10D_array_to_dict(self.cov_3x2pt_cng_10D, self.probe_ordering)
            ...
            self.cov_3x2pt_g_10D = mm.cov_10D_dict_to_array(
                cov_3x2pt_g_10D_dict, self.nbl_3x2pt, self.zbins, n_probes=2)
            self.cov_3x2pt_ssc_10D = mm.cov_10D_dict_to_array(
                cov_3x2pt_ssc_10D_dict, self.nbl_3x2pt, self.zbins, n_probes=2)
            self.cov_3x2pt_cng_10D = mm.cov_10D_dict_to_array(
    ...
    def ssc_integral_julia(self, d2CLL_dVddeltab, d2CGL_dVddeltab, d2CGG_dVddeltab,
                           cl_integral_prefactor, sigma2, z_grid, integration_type,
